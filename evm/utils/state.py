...
def make_trie_root_and_nodes(transactions, trie_class=HexaryTrie):
    chaindb = BaseChainDB(MemoryDB(), trie_class=trie_class)
    db = chaindb.db
...
def update_witness_db(witness, recent_trie_nodes_db, account_state_class):
    witness_kv = dict([(keccak(value), value) for value in witness])
    witness_kv = merge(witness_kv, recent_trie_nodes_db)
